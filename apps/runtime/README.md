...
- **`bsp-adapter.ts`** — `BspAdapter` interface plus `createMockBspAdapter()` (in-memory, records sent messages). No real 360dialog/Twilio adapter exists yet — same "wired, not live" pattern as `packages/eval`'s OpenAI client.
