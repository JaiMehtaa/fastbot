...
`generate`/`score` are injected, not hardcoded to a provider — that's what makes the orchestrator itself fully unit-testable without a live LLM call. `openai-client.ts` is the thin real-provider adapter (OpenAI), used to build real `generate`/`score` functions once `OPENAI_API_KEY` is set — it fails loudly at construction time if the key is missing, not on first use.
